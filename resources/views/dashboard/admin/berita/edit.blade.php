...
<div class="bg-white shadow rounded-xl p-6">
  <h2 class="text-lg font-semibold mb-6">Edit Berita</h2>

  <form action="{{ route('beritas.update', $berita) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
      ...
      <input type="text" name="title" value="{{ old('title', $berita->title) }}" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-indigo-200">
    </div>

    {{-- KONTEN --}}
    <div>
      <label class="block mb-1 text-sm font-medium">Deskripsi Konten</label>
      <textarea name="description" rows="6" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-indigo-200">{{ old('content', $berita->description) }}</textarea>
    </div>
    ...
    <div>
      ...
      @if($berita->image)
        ...
        <img src="{{ asset('storage/'.$berita->image) }}" class="h-32 rounded mt-2 shadow">
      @endif
    </div>
    ...
    <div>
      ...
      <input type="text" name="author" value="{{ old('author', $berita->author) }}" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-indigo-200">
    </div>

    {{-- TANGGAL TERBIT --}}
    <div>
      <label class="block mb-1 text-sm font-medium">Tanggal Terbit</label>
      <input type="date" name="published_at" value="{{ old('published_at', optional($berita->published_at)->format('Y-m-d')) }}" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-indigo-200">
    </div>
