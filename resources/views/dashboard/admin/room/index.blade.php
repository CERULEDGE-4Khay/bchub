                ...
                <tr>
                  ...
                  <td class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent w-fit">
                    ...
                    <form action="{{ route('rooms.destroy', [$room]) }}" method="POST" onsubmit="return confirm('Yakin hapus inventory ini dari ruangan?')" class="inline-flex">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Hapus</button>
                    </form>
                  </td>
                </tr>
              @endforeach
